...
export default function Hero() {
    ...
    return (
        <div id='home' className={styles.heroSection + ' ' + 'relative text-my-white w-full h-full overflow-hidden'}>
            <Image className={'z-10 left-0 h-screen w-screen object-cover '} width={200} height={200} src={'/images/LAPTOPCOFFEE.webp'} alt={'Image de fond. Un ordinateur portable et une tasse de café attendent sur une table qu\'orange un soleil couchant.'}></Image>
            ...
            <nav role="navigation" aria-expanded={mobileMenuVisible} className={'bg-black/90 z-40 flex flex-col w-full h-full items-center justify-center ' + (mobileMenuVisible ? 'absolute top-0 left-0' : 'hidden')}>
                {navItems.map(item => {
                    return (
